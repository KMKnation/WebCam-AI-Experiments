...
#    def create_model(self):
    def __call__(self):
        ...
        # genders = 3
        # max_age = 81
        predictions_g = Dense(units=genders, kernel_initializer=self._weight_init, use_bias=self._use_bias,
                              kernel_regularizer=l2(self._weight_decay), activation="softmax",
                              name="pred_gender")(flatten)
        predictions_a = Dense(units=max_age, kernel_initializer=self._weight_init, use_bias=self._use_bias,
                              kernel_regularizer=l2(self._weight_decay), activation="softmax",
                              name="pred_age")(flatten)
        model = Model(inputs=inputs, outputs=[predictions_g, predictions_a])

        pred_g = Dense(units=genders, activation="softmax",
                              name="pred_gender")(flatten)
        pred_a = Dense(units=max_age, activation="softmax",
                              name="pred_age")(flatten)
        model = Model(inputs=inputs, outputs=[pred_g, pred_a])
